...
const PROFILES: Record<string, DomainProfile> = {
  "tekstil-bilgileri": {
    ...
    glossary: {
      ...
      İplik: "Elyafların bükülmesiyle oluşan tekstil malzemesi; dokuma ve örmede kullanılır",
  ...
  for (const query of shuffledQueries) {
    ...
    try {
      const res = await fetch(
        ...
        { headers: { "User-Agent": "TekstilKOS/1.0" } }
      );
      if (!res.ok) continue;
...
function buildSections(node: any, profile: DomainProfile, children: any[], siblings: any[], rng: () => number): any {
  ...
  const m = profile.machines;
  const mat = profile.materials;
  const proc = profile.processes;
  const app = profile.applications;
  ...
  sections["siniflandirma"] = {
    title: "Sınıflandırma",
    type: "table",
    headers: ["Kategori", "Özellik", "Kullanım Alanı", "Tipik Değer"],
    rows: mat.slice(0, 5).map((material, i) => [
      material,
      p[i % p.length],
      app[i % app.length],
      i % 2 === 0 ? `${15 + i * 10}-${25 + i * 10} g/m²` : `${2 + i}-${5 + i} %`,
    ]),
  };
  ...
  sections["teknik_parametreler"] = {
    ...
    type: "table",
    headers: ["Parametre", "Birim", "Tipik Değer", "Ölçüm Standardı"],
    rows: p.slice(0, 6).map((prop, i) => [
      prop,
      i % 2 === 0 ? "g/m²" : "%",
  ...
  sections["standartlar_ve_normlar"] = {
    ...
    type: "table",
    headers: ["Standart", "Kapsam", "Referans"],
    rows: s.slice(0, 6).map((std, i) => [std, `${title} kalite kriterleri`, profile.references[i % profile.references.length]]),
  };
  ...
  sections["gorsel_sema"] = {
    title: "Görsel Şema",
    type: "diagram",
    prompt: `${title} sürecinin akış şeması: ${proc.slice(0, 5).join(" → ")} adımlarını gösteren teknik diyagram`,
...
function buildQuiz(node: any, profile: DomainProfile, children: any[], siblings: any[], rng: () => number): QuizQuestion[] {
  ...
  const proc = profile.processes;
  const app = profile.applications;
  const gloss = profile.glossary;

  // Same-module distractors: plausible but wrong answers drawn from the SAME module's data
  // These are not from other modules — they are variations within this module's domain
  const sameModuleDistractors = {
    ...
    wrongMachines: [m[1] || m[0], m[2] || m[0], m[3] || m[1] || m[0]],
  ...
  // 1. Definition — wrong options are same-module but incorrect definitions
  ...
  addQuestion({
    ...
    options: defOpts,
    correct_index: defOpts.findIndex((o) => o === desc),
    explanations: defOpts.map((o) => (o === desc ? `Doğru: "${title}" tanımı budur. ${desc}` : `Yanlış: ${title} bir ${o.includes("kalite kontrol") ? "kalite kontrol yöntemi" : o.includes("optimizasyon") ? "optimizasyon tekniği" : "standart"} değildir; ${title} ${moduleTitle(profile)} alanının bir konusudur.`)),
  });

  // 2. Classification — wrong options are sibling topics from same module
  if (children.length >= 2 || sameModuleDistractors.wrongChildren.length >= 2) {
    ...
    const wrongChildren = sameModuleDistractors.wrongChildren.length >= 3
      ? sameModuleDistractors.wrongChildren
      : [...sameModuleDistractors.wrongChildren, ...sameModuleDistractors.wrongDefs.slice(0, 3 - sameModuleDistractors.wrongChildren.length)];
    ...
    addQuestion({
      ...
      options: opts,
      correct_index: opts.findIndex((o) => o === correctChild),
      explanations: opts.map((o) => (o === correctChild ? `Doğru: "${correctChild}", "${title}" konusunun bir alt başlığıdır.` : `Yanlış: "${o}" "${title}" kapsamında yer almaz; aynı modülün farklı bir konusudur.`)),
    });
  }

  // 3. True/False — statements about this topic, using same-module terms
  ...
  addQuestion({
    ...
    options: tfOpts,
    correct_index: tfOpts.findIndex((o) => o === true1),
    explanations: tfOpts.map((o) => {
      if (o === true1) return `Doğru: ${title} sürecinde ${p[0]} kritik bir parametredir ve sürekli ölçülür.`;
      if (o === true2) return `Doğru: ${title}, ${s[0]} standardına göre değerlendirilir, ancak soruda tek doğru cevap aranmaktadır.`;
      if (o === false1) return `Yanlış: ${title} sürecinde ${k[0]} temel bileşendir; ${k[1] || k[0]} alternatif değildir.`;
      return `Yanlış: ${title}, ${proc[0]} adımını içerir; bu adım sürecin temel basamaklarındandır.`;
    }),
  });

  // 4. Comparison — compare with sibling from same module
  if (sameModuleDistractors.wrongChildren.length >= 1) {
    ...
    addQuestion({
      ...
      question: `"${title}" ve "${sib}" arasındaki ilişki için hangisi doğrudur?`,
      options: opts,
      correct_index: opts.findIndex((o) => o === correct),
      explanations: opts.map((o) => (o === correct ? `Doğru: ${title} ve ${sib} aynı modülün farklı konularıdır; birbirinden bağımsızdır.` : `Yanlış: ${title} ve ${sib} eş anlamlı veya alt basamak değildir; aynı modülün paralel konularıdır.`)),
    });
  }

  // 5. Application — wrong options are other applications from same module
  ...
  addQuestion({
    ...
    options: appOpts,
    correct_index: appOpts.findIndex((o) => o === app[0]),
    explanations: appOpts.map((o) => (o === app[0] ? `Doğru: ${app[0]}, ${title} uygulamasının birincil kullanım alanıdır.` : `Yanlış: ${o}, ${title} için birincil uygulama alanı değildir; aynı modülün farklı bir uygulamasıdır.`)),
  });

  // 6. Technical properties — wrong options are other properties from same module
  ...
  addQuestion({
    ...
    options: propOpts,
    correct_index: propOpts.findIndex((o) => o === p[0]),
    explanations: propOpts.map((o) => (o === p[0] ? `Doğru: ${p[0]}, ${title} kapsamında ölçülen temel bir teknik parametredir.` : `Yanlış: ${o}, ${title} için birincil teknik parametre değildir; aynı modülün farklı bir parametresidir.`)),
  });

  // 7. Manufacturing process — wrong options are other processes from same module
  ...
  addQuestion({
    ...
    options: procOpts,
    correct_index: procOpts.findIndex((o) => o === proc[0]),
    explanations: procOpts.map((o) => (o === proc[0] ? `Doğru: ${proc[0]}, ${title} sürecinin temel adımlarındandır.` : `Yanlış: ${o}, ${title} sürecinde yer almaz; aynı modülün farklı bir sürecidir.`)),
  });
  ...
  addQuestion({
    ...
    options: advOpts,
    correct_index: advOpts.findIndex((o) => o.startsWith(p[0])),
    explanations: advOpts.map((o) => (o.startsWith(p[0]) ? `Doğru: ${title}, ${p[0]} açısından üstünlük sağlar ve ürün kalitesini artırır.` : `Yanlış: ${o}, ${title} avantajlarından biri değildir; bu aynı modülün farklı bir özelliğidir.`)),
  });

  // 9. Disadvantages — wrong options are same-module machines/processes
  ...
  addQuestion({
    ...
    options: disOpts,
    correct_index: disOpts.findIndex((o) => o.startsWith(m[0])),
    explanations: disOpts.map((o) => (o.startsWith(m[0]) ? `Doğru: ${m[0]} yatırım maliyeti, ${title} uygulamasının bir dezavantajıdır.` : `Yanlış: ${o}, ${title} için spesifik bir dezavantaj değildir; aynı modülün farklı bir zorluğudur.`)),
  });
  ...
  addQuestion({
    ...
    options: caseOpts,
    correct_index: caseOpts.findIndex((o) => o.startsWith(app[0])),
    explanations: caseOpts.map((o) => (o.startsWith(app[0]) ? `Doğru: ${app[0]} alanında ${title} uygulaması ${p[0]} iyileştirmesi sağlar.` : `Yanlış: ${o}, ${title} vaka sonuçlarından biri değildir; aynı modülün farklı bir uygulama sonucudur.`)),
  });

  // 11-15. Additional questions from children and glossary
  children.slice(0, 3).forEach((child) => {
    ...
    addQuestion({
      ...
      options: opts,
      correct_index: opts.findIndex((o) => o === correct),
      explanations: opts.map((o) => (o === correct ? `Doğru: ${child.title}, ${title} konusunun bir alt başlığıdır.` : `Yanlış: ${o}, ${title} alt başlığı değildir; aynı modülün farklı bir konusudur.`)),
    });
  });

  // Glossary-based questions
  if (questions.length < 13 && Object.keys(gloss).length >= 2) {
    ...
    addQuestion({
      ...
      options: opts,
      correct_index: opts.findIndex((o) => o === def),
      explanations: opts.map((o) => (o === def ? `Doğru: "${term}" tanımı: ${def}` : `Yanlış: Bu tanım "${term}" için geçerli değildir; aynı modülün farklı bir terimine aittir.`)),
    });
  }

  // Materials question
  if (questions.length < 14 && mat.length >= 4) {
    ...
    addQuestion({
      ...
      options: opts,
      correct_index: opts.findIndex((o) => o === mat[0]),
      explanations: opts.map((o) => (o === mat[0] ? `Doğru: ${mat[0]}, ${title} sürecinde kullanılan temel malzemedir.` : `Yanlış: ${o}, ${title} sürecinde birincil malzeme değildir; aynı modülün farklı bir malzemesidir.`)),
    });
  }
  ...
  if (questions.length < 15 && s.length >= 2) {
    ...
    addQuestion({
      ...
      options: opts,
      correct_index: opts.findIndex((o) => o === s[0]),
      explanations: opts.map((o) => (o === s[0] ? `Doğru: ${s[0]}, ${title} kapsamında referans alınan standarttır.` : `Yanlış: ${o}, ${title} için referans standart değildir; aynı modülün farklı bir standardıdır.`)),
    });
  }
...
function moduleTitle(profile: DomainProfile): string {
  return profile.moduleSlug.replace(/-/g, " ");
}
