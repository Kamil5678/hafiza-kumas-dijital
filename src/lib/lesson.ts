...
export async function fetchNotes(slug: string): Promise<string> {
  const { data } = await supabase.from("lesson_notes").select("note").eq("node_slug", slug).order("updated_at", { ascending: false }).limit(1);
  return data && data.length > 0 ? data[0].note : "";
}

export async function saveNote(slug: string, note: string): Promise<void> {
  const { data: existing } = await supabase.from("lesson_notes").select("id").eq("node_slug", slug).limit(1);
  if (existing && existing.length > 0) {
    await supabase.from("lesson_notes").update({ note, updated_at: new Date().toISOString() }).eq("id", existing[0].id);
  } else {
